...
class FundusDataHandler(object):
  ...
  """
    ...
    def get_allQuality_files(self):

        files_fix = []
        files_mov = []

        paths = glob.glob("DataBases/allQuality/croped_images/*.png")
        j = 0
        fix = []
        mov = []
        for i in paths:
        ...
        files_fix.append(fix)
        files_mov.append(mov)

        return  files_fix, files_mov

    def get_paiva_files(self):
    ...
    def get_DRIVE_files(self):
        ...
        pathsf = glob.glob("DataBases/DRIVE/test/croped_images/*.png")
        pathsm = glob.glob("DataBases/DRIVE/training/croped_images/*.png")


        files_fix.append(pathsf)
        files_mov.append(pathsm)
    ...
    def sample_pair_bases(self, i):
        x = []
        y = []

        x.append(self.fire_fix[0][i])
        y.append(self.fire_mov[0][i])
